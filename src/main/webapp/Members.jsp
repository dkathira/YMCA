...
<body>
	...
	<div class="innercontainer">
		<div class="sidebar">
			<a href="StaffLogin.jsp">Home</a> <a href="Programs.jsp">Programs</a>
			<a class="active" href="Members.jsp">Members</a> <a
				href="Helppage.jsp">Help</a>
		</div>
		<div class="content">
			<!--  <div>
			
				<form class="example" action="MemberSearch.jsp"
    ...
    alert(this.id);
			...
			<div class="container">
				...
				try {
					...
					String sql = "select * from ymca_program.users";
					...
					String sql2 = "select ProgramID from ymca_program.joined_programs where UserID =" + uid;
					resultset2 = statement2.executeQuery(sql2);
					count = 0;
					while (resultset2.next()) {
						String u = resultset2.getString("ProgramID");
						statement3 = connection.createStatement();
						String sql3 = "select ProgramName,StartDate,StartTime,Days,ActiveStatus from ymca_program.program where ProgramID ="
								+ u;
						resultset3 = statement3.executeQuery(sql3);

						if (resultset3 != null) {
							while (resultset3.next()) {
								String PName = resultset3.getString("ProgramName");
								String Date = resultset3.getString("StartDate");
								String Time = resultset3.getString("StartTime");
								String Days = resultset3.getString("Days");
								String Status = resultset3.getString("ActiveStatus");
								int status = Integer.parseInt(Status);
								if (status == 1) {
									Final[count] = PName + " from " + Date + " at " + Time + " on " + Days;
									count++;
								}
							}
						}
				...
				<div class="row"
					style="background-color: #f1f1f1; height: 240px; max-width: 100%">
					<input type="hidden" id="programid" name="programid"
						value=<%=resultset.getInt("UserID")%> /> <input type="hidden"
					...
					<div class="col-8">
						<table style="float: left; width: 80%">
							...
							<tr>
								...
								<td><%=resultset.getString("EmergencyContact")%></td>
								<td><b>Emergency Contact No : </b></td>
								<td><%=resultset.getString("EmergencyContactNo")%></td>
							</tr>
							<tr>
								<td><b>Health Issues : </b></td>
								<td><%=resultset.getString("HealthIssues")%></td>
							</tr>
							<%
							if (count != 0) {
							%>
					...
					<div class="col-2 menu">
						<form method="post" action="<%=request.getContextPath()%>/AddUser"
							...
							if (activestatus != 0) {
							%>
							<button type="submit" class="button2" value="Deactivate"
								id="Deactivate" form="deleteform">Deactivate User</button>
							<%
							action = "deactivate";
							} else {
							%>
							<button type="submit" class="button1" value="Activate"
								id="Activate" form="deleteform">Activate User</button>
							<%
							action = "activate";
							}
							%>
							...
							%>
							<button type="submit" class="button2" value="Deactivate"
								id="Deactivate" form="deleteform">Cancel Membership</button>
							<%
							action = "cancel";
							} else {
							%>
							<button type="submit" class="button1" value="Activate"
								id="Activate" form="deleteform">Give Membership</button>
							<%
							action = "give";
							}
							%>
							<input type="hidden" id="deleteid" name="deleteid"
								value=<%=resultset.getInt("UserID")%> /> <input type="hidden"
								id="activestatus" name="activestatus" value=<%=activestatus%> />
							<input type="hidden" id="memberID" name="memberID"
								value=<%=memberID%> /> <input type="hidden" id="action"
	...
	<div class="footer">
		<hr class="solid">
		<div style="float: right; color: #3090C7; font-size: 20px;">
			Contact US</div>
	</div>
</body>
</html>
